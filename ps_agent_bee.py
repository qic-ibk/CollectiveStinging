...
class BasicPSAgent(object):
	...
	def __init__(self, num_actions, num_percepts_list, gamma_damping, eta_glow_damping, policy_type, beta_softmax, num_reflections,init_mode,init_psting):
		...
		if self.init_mode=='standard':
			...
			self.h_matrix[0,:]=(1-self.init_psting)*np.ones(self.num_percepts, dtype=np.float64)
			self.h_matrix[1,:]=self.init_psting*np.ones(self.num_percepts, dtype=np.float64)
			self.h0_matrix=np.ones((self.num_actions, self.num_percepts), dtype=np.float64)
			self.h0_matrix[0,:]=(1-self.init_psting)*np.ones(self.num_percepts, dtype=np.float64)
			self.h0_matrix[1,:]=self.init_psting*np.ones(self.num_percepts, dtype=np.float64)
